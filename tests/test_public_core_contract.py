...
def test_public_core_node_is_registered_and_business_neutral():
    ...
    for forbidden_input in (
        "turbo_lora", "style_lora_stack", "prompt", "seed",
        "resolution", "sampler", "sigmas",
    ):
        assert f'"{forbidden_input}"' not in node
...
def test_master_loader_remains_integrated_cockpit():
    ...
    master = text[start:]
    for expected in (
        '"unet_name"', '"turbo_lora"', '"steps"', '"duration_seconds"',
        '"resolution"', '"seed"', '"prompt"', '"style_lora_stack"',
    ):
...
def test_prebuilt_mode4_uses_steps_hint_not_stock_20_contract():
    ...
    assert 'expected_steps=20' not in block
